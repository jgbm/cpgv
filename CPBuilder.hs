...
instance ChannelName LIdent
    where binder (LIdent s) = binder s
          reference (LIdent s) = reference s
instance ChannelName String
    where binder s b = B (\m z -> let s' = s ++ show z
          ...
          reference s = B (\m z -> case lookup s m of
                                     Just s' -> (LIdent s', z)
                                     Nothing -> (LIdent s, z))
instance ChannelName t => ChannelName (Builder t)
    where binder b c = b >>= flip binder c
...
request x y m = do x' <- reference x
                   binder y (\y' -> liftM (ClientRequest y') m)
