...
pterm (Fork x a t) = prec 1 (group (align (hang 2 ("fork" <+> text x <> colon <+> pretty a<> dot <$> pretty t))))
pterm (Serve x a t) = prec 1 (group (align (hang 2 ("serve" <+> text x <> colon <+> pretty a <> dot <$> pretty t))))
