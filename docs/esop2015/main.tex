...
\tocp{\gvsend{(M,N)}}z &=&
  \cut{x}{}{\tocp{N}x}{\cut{y}{}{\tocp{M}y}{x[w].(\link{w}{y} \mid \link{x}{z})}} \\
\tocp{\gvreceive{M}}z &=& \tocp{M}z \\
\tocp{\gvwait{M}}z    &=& \tocp{M}z \\
\tocp{\lambda^\sigma x.M}z &=& \tocp{\sigma}(z(x).\tocp{M}z) \\
\tocp{L \app M}z &=&
%  \cut{x}{}{\tocp{M}x}{\cut{y}{}{\tocp{L}y}
%     {\cut{w}{}{\link{y}{w}}{x[v](\link{v}{w} \mid \link{x}{z})}}} \\
%%  \cut{x}{}{\tocp{M}x}{\cut{y}{}{\tocp{L}y}{x[w].(\link{y}{w} \mid \link{x}{z})}} \\
  \cut{x}{}{\tocp{M}x}{\cut{y}{}{\tocp{L}y}{y[w].(\link{w}{x} \mid \link{y}{z})}} \\
\tocp{()}z    &=& z[] \\
...
\item Case $\Fx[\gvconst{send} \app (V,x)] \parallel \Fx'[\gvconst{receive} \app x] \ceval \Fx[x]
  \parallel \Fx'[(V,x)]$.
%
By Lemma~\ref{bla} we need only consider the case where $\Fx = \Fx' = [~]$.
\[
\bl
