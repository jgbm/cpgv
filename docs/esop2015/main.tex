...
\figref{gv-syntax} gives the syntax of GV types and terms.  The types $T$ include nullary and binary
...
provided by \gvconst{fork}.  If $f$ is a function from a session of type $S$ to a closed session (of
...
must be closed by calling \gvconst{wait}.  A client of the process above could be
%
\[N \defeq
  \bl
...
\gvconst{wait} synchronizes with the produced session. We do not incorporate equivalence directly in
