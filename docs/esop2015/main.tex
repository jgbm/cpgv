...
From massively distributed programs, running across entire datacenters, to apps running on handheld
devices, reliant on remote services for functionality, concurrency has become a central problems in
program correctness. Assuring correct concurrent behavior requires reasoning not just about the
types of data communicated, but the order in which the communication takes place. For a simplistic
example, the exchange between an SMTP client and server is all textual; however, a client that sends
the recipient's address before the sender's address is still in violation of the protocol, despite
...
continue with the behavior specified by $S$. An important property of session types is duality: from
the type of the SMTP client, we can immediately derive the type of the server:
\[
...
details.) We present a small-step operational semantics for GV, factored into $\lambda$-calculus and
concurrent portions following the approach of Gay and Vasconcelos~\cite{GayVasconcelos10}. The
$\lambda$-calculus portion of our semantics differs from standard presentations of CBV reduction
...
setting; this is somewhat non-trivial because closed expressions may return channels, so we cannot
...
Wadler's calculus CP, and show that these translations preserve not just typing, but reduction.
Finally, we show that GV is a useful basis for further language development, by giving two
extensions of GV and showing that our deadlock freedom results transfer easily to the extended
languages.
...
\item We connect our language to the interpretation of session types as linear logic propositions,
  ...
  begin by introducing CP~\secref{cp}. We show that we can simulate CP reduction in
  GV~\secref{togv}, and GV reduction in CP~\secref{tocp}. As process calculi provide substitution
...
\begin{syntax}
  ...
  \text{Types} & T,U & ::= & S \mid T \gvtimes U \mid \one \mid T \gvplus U \mid \zero \mid T \lto U
  \\ % \mid {!T} \\
  \text{Terms} & L,M,N & ::= & x \mid K \mid \lambda x.M \mid M\,N \\
...
and sums (there is, of course, no term to construct a 0-ary sum).  Typing rules, including the type
schema for the constants $K$, are given in \figref{gv-typing}.
...
and \gvconst{receive}.  For example, assuming an extension of our core language with numbers and
arithmetic operators, a computation that received a pair of numbers along a channel $z$ and then
...
\gvin{(Int \gvtimes Int)}{\gvout{Int}{S}}$ at its start, and evaluates to a session of type
$S$. Session initiation is provided by \gvconst{fork}.  If $f$ is a function from a session of type
$S$ to a closed session (of type $\outterm$), then $\gvconst{fork} \app f$ runs $f$ and returns a
session to communicate with it.  For example, the term
%
\[\bl
...
\el\]
%
returns a channel of type $\gvout{(Int \gvtimes Int)}{\gvin{Int}{\interm}}$.  In a process created
by $\gvconst{fork} \app f$, the session returned from $f$ is closed by $\gvconst{fork}$; however,
the other end of the channel must be closed by calling \gvconst{wait}.  A client of the process
above could be
%
\[\bl
...
\el\]
%
The combination of these processes would evaluate to 13.  Finally $\gvconst{link} \app (x, y)$
causes messages sent on $x$ to be received on $y$ and vice versa.
...
they are macro-expressible using the term-level sum construct.  The intuition is that selection is
...
\end{align*}
%
While we would expect a surface language to include selection and branching, we will omit them, as
...
do not).  We designate the main thread in a configuration by $\distinguish M$ and forked threads by
$\thread M$.
...
\figref{gv-reduction}. Term reduction ($\teval$) is typical call-by-value left-to-right evaluation;
...
\gvconst{wait} synchronizes with the produced session. Atypically, we do not include a structural
...
In this section, we show that GV can simulated CP. \figref{from-cp} gives the translation of CP
...
In this section, we show that CP can simulate GV. Our translation differs from both that of
Wadler's~\cite{Wadler14} and our prior work~\cite{LindleyM14}. We will elaborate on the differences
below. \figref{from-gv-types} gives the translation on types and \figref{from-gv-terms} gives the
translation on terms, substitutions, and configurations; we translate typing environments by the
pointwise extension of the translation on types.
%
...
%
In the translation of configurations, we write $C \parallel_x C'$ to indicate that the variables $x$
is shared by $C$ and $C'$; in a well-typed GV configuration, there will always be exactly one such
variable, so the translation is unambiguous.
...
axiom cuts. We have highlighted these in grey in the definition. We included them in order to ensure
that the proof goes through.
%
The output in the translation of $\mkwd{fork}$ arises from the need to apply the argument to a
freshly generated channel (notice that application is simulated by an output). The $\mkwd{send}$
operation is simulated by output as one might expect; $\mkwd{receive}$ is simulated by input
composed with outputting the result to the return channel. The $\mkwd{wait}$ operation is simulated
by simply connecting the result to the return channel.

\todo{SL: still half way through doing this...}

Variables are linked to the return channel $z$. Closures are simulated by input, subject to an
...
\begin{theorem}
If $\Gamma \vdash M : T$ then $\tocp{M}z \vdash \tocp{G}, z:\tocp{T}$.
\end{theorem}

\todo{Explain the translation on terms}


The translation on terms presented here, diverges somewhat from the old translation presented
previously~\cite{Wadler14,LindleyM14}. One problem with the old translation is that it does not
reflect $\beta$-reduction. The fundamental reason for this is that CP cannot simulate substitution
under a lambda abstraction, and this is why we adapted GV to use weak explicit substitutions.
%
