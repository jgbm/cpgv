...
correctness. Assuring correct concurrent behaviour requires reasoning not just about the types of
...
\item We define a core linearly-typed functional language, by extending linear lambda calculus with
  ...
  and terminating~\secref{determinism} We characterise deadlock and normal forms; we show that typed
...
\item We consider two extensions of our core language: one which has a single, self-dual closed
  channel, harmonising the treatment of closed channels with their treatment in other session-typed
  calculi~\secref{gv-end}, and another which adds unlimited types and replicated
  behavior~\secref{gv-re pl}. We show that these extensions preserve the essential deadlock-freedom
...
system. We now present a general account of deadlock: we characterise deadlocked configurations, and
...
$\with$ against $\oplus$ corresponds to the synchronisation of an internal and external choice.  The
rule for reduction of $\parr$ against $\otimes$ is more complex than synchronisation of input and
...
rely on our encoding of branching and choice in GV~\secref{gv-static}.
...
directly apply our characterisations of deadlock and deadlock freedom to show that the extended
...
session $w$, however, there can be no further synchronisation between the threads; in contrast, in
core GV, there is always a final synchronisation with \gvconst{wait}.  To capture the increase in
