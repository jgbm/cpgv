...
\begin{syntax}
  ...
  \text{Configuration contexts} & \Cx & ::= & [~] \mid \Cx \parallel C \mid (\nu x)\Cx \\
...
\todo{Add $\nu$s back into reduction rules and get rid of reduction on type environments.}
...
% For well typed terms, the relation $\equiv\ceval\equiv$ satisfies the diamond property:
  If $\Gamma \vdash C$, $C \equiv\ceval\equiv D_1$, and $C \equiv\ceval\equiv D_2$, then either $D_1
  \equiv D_2$ or there exists $D_3$ such that $D_1 \equiv\ceval\equiv D_3$, and $D_2
  \equiv\ceval\equiv D_3$.
\end{theorem}
\begin{proof}
...
\todo{Remark on existing (partial) confluence results for linear pi calculus and sessions}

%% Reviewer A:
%%
%%   It should also be remarked that (partial) confluence of communications on linear channels is a
%%   known property already investigated in [14] for the linear pi calculus and, as a by product of
%%   [7], extendable to communications on (binary) session channels
...
%
\todo{Try to give an example illustrating the need for the trivial axiom cuts.}
%
...
\el\]
%
Initially, the forked thread and its parent share (only) channel $w$.  After both threads close $w$,
...
composition in which no channels are shared (such as after the reduction of $\gvconst{close}\app w$
in the example):
\[
\inferrule
  {\Gamma \vdash^\phi C \\
...
\begin{align*}
  ...
  Service(S) &\defeq \gvin{(S \to \outterm)}{\interm}
\end{align*}
...
\begin{proof}
  ...
  \item In case \textsc{Link}, from the assumptions $\Gamma \vdash (\nu x) (\Fx[\gvconst{link} \app
    ...
    reduction rule for \gvconst{link} allows us to assume that neither $\Fx$ nor $\Fx'$ binds $x$ or
