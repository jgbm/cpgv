...
\begin{equations}
\tocp{x}z &=& \link{x}{z} \\
\tocp{\gvconst{fork} \app V}z &=& \cut x {} {\cut y {} {\tocp{M}y} {y[x].y[].0}} {\link x z} \\
\tocp{\gvlink{M}{N}}z &=& z().\cut{x}{}{\tocp{M}x}{\tocp{N}x} \\
%\tocp{\gvsend{(M,N)}}z &=& \cut{x}{}{\cut{y}{}{\tocp{M}y}{x[y].\link{x}{z}}}{\tocp{N}x} \\
\tocp{\gvsend{(M,N)}}z &=& \cut{y}{}{\tocp{M}y}{\cut{x}{}{\tocp{N}x}{y[x].\link{y}{z}}} \\
\tocp{\gvreceive{M}}z &=& \tocp{M}z \\
\tocp{\gvwait{M}}z    &=& \tocp{M}z \\
\tocp{\lambda^\sigma x.M}z &=& \cut {x_1} {} {\tocp{V_1}x_1} {\dots \mid \cut {x_n} {} {\tocp{V_n}} {z(x).\tocp{M}z} \dots} \\
&& \text{ if $\sigma = \{ \bind {x_1} {V_1}, \dots, \bind {x_n} {V_n} \}$} \\
\tocp{M \app N}z &=&
  \cut{y}{}{\tocp{M}y}{\cut{x}{}{\tocp{N}x}{y[x].\link{y}{z}}} \\
%% \tocp{L \app M}z &=&
%%     \cut y {} {\tocp{L}y}{y[x].(\tocp{M}x \mid \link{y}{z})} \\
\tocp{()}z    &=& z[] \\
\tocp{(M,N)}z &=& z[y].(\tocp{M}y \mid \tocp{N}z) \\
\tocp{\gvlet{()}{M}{N}}z &=& \cut y {} {\tocp{M}y} {y().\tocp{N}z} \\
\tocp{\gvlet{(x,y)}{M}{N}}z &=& \cut{y}{}{\tocp{M}y}{y(x).\tocp{N}z} \\
\tocp{\mkwd {inl} \app M}z &=& z[\mkwd{inl}].\tocp{M}z\\
\tocp{\mkwd {inr} \app M}z &=& z[\mkwd{inr}].\tocp{M}z\\
