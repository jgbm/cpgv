...
From the biggest programs, running across entire datacenters, to programs running on handheld
...
types of data commuicated, but the order in which the communication takes place.  For a simplistic
example, the exchange between an SMTP client and server is all textual; however, a client that sends
the receipient's address before the sender's address is still in violation of the protocol.
...
continue with the behavior specified by $S$.  One important property of session types is duality:
from the type of the SMTP client, we can immediately derive the type of the server:
\[
...
continue with the behavior specified by $S$.

Session types were originally developed for process calculi, such as the $\pi$-calculus.  Some
recent work has adapting session types to more practical programming languages.  In this paper, we
are particularly interested in the use of session types for concurrency in functional programming
languages, a developed by Vasconcelos et al.~\cite{VasconcelosGR06} and by Gay and
Vasconcelos~\cite{GayVasconcelos10}.

Session type systems are crucially substructural---if processes could freely discard or duplicate
...
session-typed process calculus.  Walder~\cite{Wadler12} adapted their approach to classical linear
logic, emphasizing the role of duality in the typing rules; the semantics of his system are given
directly by the cut elimination rules for classical linear logic.  He also gave a type-preserving
...
language shares many of the distinctive features of Wadler's, although it differs in some
not-insigificant details.)  We develop a small-step operational semantics for GV, factored into
$\lambda$-calculus and concurrent portions following the approach of Gay and
Vasconcelos~\cite{GayVasconcelos10}.  The $\lambda$-calculus portion of our semantics differs from
standard presentations of CBV reduction only in that we need a weak form of explicit substitution to
maintain a connection with cut reduction.  The concurrent portion of our semantics includes the
typical reductions and equivalences of $\pi$-calculus-like systems.  We develop a characterization
of deadlock-free programs in our setting; this is complicated because closed expressions may return
channels, so we cannot simply require that all possible communication take place.  We show that
well-typed GV programs are deadlock-free, deterministic, and terminating.  We give translations
between our language GV and Wadler's calculus CP, and show that these translations preserve not just
typing, but reduction.  Finally, we show that GV is a useful basis for further language development,
by giving two extensions of GV and showing that our deadlock freedom results transfer easily to the
extended languages.
...
\item We connect our language to the interpretation of session types as linear logic propositions,
  ...
  calculus CP.  We begin by introducing CP~\secref{cp}.  We show that we can simulate CP reduction
  in GV~\secref{togv}, and GV reduction in CP~\secref{tocp}.  As process calculi provide
  substitution only for names, not entire process expressions, the latter depends crucially on the
  use of weak explicit substitutions in the semantics of GV $\lambda$ terms.
...
with the reduction relation $\Gamma \ceval \Gamma'$.  We write $\Gamma \ceval^? \Gamma'$ for the
reflexive closure of $\ceval$.
...
normalisation also follows as a direct corollary of \thmref{gv-reduction-simulated} and the
...
In this section, we show that CP can simulated GV. \figref{from-gv} gives the translation of GV
...
\todo{Explain $\parallel_x$ notation; do we actually use this anywhere else? Yes, in the proofs.}
...
the compositionality of the definitions and \thmref{teval-preserved}. We give a representive
...
and~\ref{th:gv-reduction-simulated} to unlimited channels is direct.
