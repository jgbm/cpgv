...
expandP ds = ex
    where ex (ProcVar v ps)    = do (vs, p) <- getBinding v (procs ds)
          ...
          ex (Cut x a p q)         = liftM3 (Cut (exn x)) (expandT ds a) (expandP (filterNameBindings (x /=) ds) p)
          ...
          ex (Roll x y a p q)      = liftM3 (Roll x y) (expandT ds a) (expandP ds' p) (expandP ds' q)
...
expandT :: Defns -> Prop -> Either String Prop
