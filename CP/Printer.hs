...
prop (FOUniv t a)  = prec 1 (group (pretty t <+> "||" <$> prop a 2))
prop (FOExist t a) = prec 1 (group (pretty t <+> "*" <$> prop a 2))
...
instance Pretty Proc
    where pretty (ProcVar v []) = name v
          ...
          pretty (Rec x p) = hang 2 ("rec" <+> name x <> dot <//> pretty p)
          pretty (CoRec x y a p q) = hang 2 (group ("corec" <+> name x <+> brackets (name y <> colon <+> pretty a) <$>
