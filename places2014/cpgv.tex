...
\begin{abstract}
Recently, Wadler demonstrated a continuation-passing translation from a session-typed functional
language, called GV, to a process calculus, called CP, based on classical linear logic.  However,
this translation is one-way: CP is more expressive than GV.  This paper proposes an extensions to
GV, called HGV, which remedies the expressiveness gap.  We demonstrate direct translations from CP
into the session fragment of HGV (which we call \hgvpi), and from HGV to \hgvpi.  We show that the
original continuation-passing translation of GV to CP factors through the direct translation to
\hgvpi.  Finally, these translations shed light both on the original translation from GV to CP, and
on the limitations in expressiveness of GV.
\end{abstract}
