...
GV has an explicit construct for handling an end input session type. This is inconvenient and
unnecessary for programming in practice, as a term of this type never carries any data. Thus,
instead we choose to make $\gvEndInput$ unlimited, admitting weakening and contraction.
%
\begin{mathpar}
...
\end{mathpar}%
%
We can straightforwardly simulate explicit termination as follows:
\[
...
The typing rule for $\key{serve}$ is anomolous because it mentions a service type but no server
type. An alternative is to define a $\key{defServere}$ construct that specifies the definition of a
server. We prefer the former rule because we find it more convenient to program with than the more
uniform alternative.
\begin{mathpar}
\infr[Serve]
{\gvj{\Phi,x : S}{M}{\gvEndOutput} \\ \un{\Phi}}
...
\end{mathpar}%
%
The two constructs are straightforwardly interdefinable:
\[
\gvServe{y}{M} \equiv \gvFork{x}{\gvServeOld{x}{y}{M}}
\qquad
\gvServeOld{M}{x}{N} \equiv \gvLink{M}{(\gvServe{x}{N})}
