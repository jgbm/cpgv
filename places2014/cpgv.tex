...
logic proofs as $\pi$-calculus processes.
%
%% While they provide $\pi$-calculus interpretations of all
%% linear logic proofs, they do not provide a proof-theoretic interpretation for arbitrary
%% $\pi$-calculus terms.
%
Caires and Pfenning~\cite{CairesPfenning10} give a Curry-Howard correspondence between
intuitionistic linear logic and session types, interpreting proofs as processes in a restricted
$\pi$-calculus, $\pi$DILL.
%
%% Caires and Pfenning~\cite{CairesPfenning10} observe that the multiplicative
%% connectives can be interpreted as session types. Their process calculus, based on intuitionistic
%% linear logic, is closer to $\pi$-calculus than are traditional session-typed
%% languages.
...
%
Furthermore, Wadler shows that a core session-typed linear functional language, GV, patterned after
...
program.
%
%% Wadler~\cite{Wadler12} shows that a core session-typed linear functional language, GV,
...
%% , suggesting that the construct we adopt here is more natural. The original construct may be
%% recovered by a combination of $\key{fork}$ and $\key{serve}$.
...
\lampi{\gvTimes{T}{U}} = \gvInput{\gvDual{\lampi{T}}}{\gvDual{\lampi{U}}}
\]%
...
We present the typing rules of CP in Figure~\ref{fig:cp-typing}. Due to lack of space we omit the
definition of the cut relation $\redto$. A detailed description of CP, including the cut rules, can
be found in Wadler's work~\cite{Wadler12}. Note that the propositions of CP are exactly those of
classical linear logic, as are the cut rules (if we ignore the terms). Thus, CP enjoys all of the
standard meta theoretic properties of classical linear logic, including confluence and weak
normalisation.
%
...
\begin{theorem}
\label{th:factor}
$\hgvcp{\lampi{M}}z \redto^* \hgvcp{M}z$ (where $\redto^*$ is the transitive reflexive closure of $\redto$).
\end{theorem}
\noindent
...
The typing rule for $\key{serve}$ is anomalous because it has a service type in its conclusion
...
%%  LocalWords:  Lindley Wadler GV CP Girard Abramsky Bellin Milner's Caires Pfenning Vasconcelos
%%  LocalWords:  intuitionistic polymorphism Wadler's Vasconcelos's duals exponentials dualisation
%%  LocalWords:  GV's SendType ReceiveType interdefinable homomorphic Ax ary nullary dualised et al
%%  LocalWords:  pointwise expressivity Felleisen's Felleisen Dardha EPSRC EP cpgv defServer
%%  LocalWords:  ServeDef
