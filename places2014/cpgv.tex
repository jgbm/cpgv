...
a core session-typed linear functional language, GV, patterned after that of Gay and
Vasconselos~\cite{GayVasconcelos10}, may be translated into a process calculus, CP, whose terms are
proofs in classical linear logic.
%
...
Wadler's GV~\cite{Wadler12}. \hgv's features are closer to those of \last, but it retains a type
system corresponding closely to linear logic. In designing \hgv, we have sometimes opted for
programming convenience over uniformity, while insisting on a tight correspondence with linear
logic.
%
...
logic, where there is no convenient self-dual proposition to represent the end of a
...
replicated session: a session of type $\gvService{S}$ is a ``service'', providing any number of
copies of a session of type $S$; a session of type $\gvServer{S}$ is the ``server'' providing that
service.
%
...
\el
\]
Note that dualisation leaves input and output types unchanged
%% , which is necessary given that the
%% notion of dual makes no sense for non-session types.
%
...
\hgv's terms and their typing rules are given in Figure~\ref{fig:hgv-typing}; the first three rules
are structural, followed by six rules for non-session types and eight rules for session-types. The
novel features of \hgv appear in the final six rules. We replace GV's
$\key{with}~\dots~\key{connect}~\dots~\key{to}$ construct by $\key{fork}$; the two are
interdefinable, but we find $\key{fork}$ more convenient. %natural and better aligned with \last.
\[
\gvFork{x}{M} \equiv \gvWith{x}{M}{x} \qquad \gvWith{x}{M}{N} \equiv \gvLet{x}{\gvFork{x}{M}}{N}
\]
We add a term $\gvLink{M}{N}$ to implement channel forwarding; this is provided in neither GV nor
\last, but is necessary to match the expressivity of CP. we add terms $\gvSendType{S}{M}$ and
$\gvReceiveType{X}{M}$ to provide session polymorphism, and $\gvServe{x}{M}$ and $\gvRequest{M}$ for
replicated sessions. Note that, as the body $M$ of $\gvServe{x}{M}$ may be arbitrarily replicated,
it can only refer to the unlimited portion of the environment.
%
...
of the standard meta theoretic properties of classical linear logic.
...
choice ($\binampersand$) types are $n$-ary, whereas he presents binary and nullary instances of
each.
...
We highlight several areas for continued research.
%% \begin{itemize}
%% \item 
The semantics of \hgv is given via cut elimination in CP. We would like to give \hgv a
  semantics directly, in terms of reductions of configurations of processes, and then prove a formal
