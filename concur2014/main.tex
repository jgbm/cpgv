...
\newcommand{\lrkwd}{\mkwd{def}}
\newcommand{\gvletrec}[3]{\lrkwd\: #1 = #2\:\mkwd{in}\:#3}
...
give a natural defintion of duality for fixed points:
%
{\small\begin{align*}
\gvdual{\mu G} &= \nu \gvdual{G} & \gvdual{\nu G} = \mu \gvdual{G}
\end{align*}}
...
implementing the equivalences. The expression $\gvletrec{f\,\vec{x}\,c}{M}{N}$ is used to define
corecursive sessions.  We will give several simple examples to motivate its behavior; the remainder
of the section will provide more in depth examples.

Consider a stream of zeros.  (We assume integer constants and addition for these examples.)
The type of such a stream is $\nu X. !Int.X$.  We would expect the definition of such a stream to
provide one number before recursing; in \mugv, we could begin implementing this by
...
\[ \lrkwd \: f\:c = f\:c.\]
To ensure productivity, the typing rule is restricted.  We use a (fresh) type variable $X$ to
abstract over the recursive behavior; the channel $c$ then has type $G(X)$ instead of $G(\nu G)$,
and $f$ has type $X \uto \outterm$ instead of $\nu G \uto \outterm$.  $M$ is thus required to
provide precisely one iteration of the recursive behavior.
...
\todo{Explanation of $\lrkwd$ that makes sense/explains anything. Note parallel: $$\mkwd{send}:T \to
  \gvout{T}{S} \to S$$ \textit{consumes} a $\gvout{T}{S}$, and the $f$ defined by $\lrkwd$
  \textit{consumes} a channel $\nu G$.}
...
of processes, each storing one bit.  The chain supports operations to increment and retreive the
...
%
\[\begin{array}{lllll}
\mcl{5}{\lrkwd\:Node\:b\:d\:c =} \\
\mcl{5}{\quad \mkwd{case}\:c\:\mkwd{of}\:\{} \\
\quad \quad & \clabel{val}\:c. & \mcl{3}{\gvlet{m}{\gvreceive{c}}{}} \\
...
\]
Value retrieval produces the value of the string, increment gives a new string, and halt gives a
closed channel.  The implementation requires one state component, the first link of the chain
(called $d$).
\[\begin{array}{llll}
\mcl{4}{\lrkwd\:CounterImpl\:d\:c =} \\
...
& & & CounterImpl\:e\:c; \\
& & \quad \clabel{done}\:e. & CounterImpl\:d\:c\:\}; \\
...
{\small\[\begin{array}{rcl@{\hspace{0mm}}ll}
  Add_{x,y,z} & = & \mcl{3}{\lrkwd\:x\langle a \rangle} \\
