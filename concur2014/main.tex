...
\usepackage{amsmath,amssymb,mathwidth,stmaryrd,xspace,url,hyperref}
...
\item Section~\ref{sec:mugv} extends GV to support recursion, yielding \mugv; and demonstrates its
  ...
  cases.
\item Section~\ref{sec:mucp} extends CP to support recursion, yielding \mucp, and presents a
...
We let $\Phi$ range over type environments. The judgement $\gvtyp{\Phi}{M}{T}$ states that term $M$
has type $T$ in type environment $\Phi$.  Figure~\ref{fig:gv-typing} gives the typing rules of
\mugv. The structural rules account for variables, and for weakening and contraction of unlimited
types. The rules for the functional terms are standard; note that, to account for linearity, the
context is split in the rules for application and pair introduction and elimination, and is
restricted to unlimited types in the introduction of unlimited arrows.

The typing of input, output, choice and selection are those of \citet{GayVasconcelos10}.  Following
our earlier work~\citep{LindleyM14}, the term $\gvlink{M}{N}$ implements channel forwarding.
...
The rule for $\gvserve{x}{M}$ parallels that for $\tkwd{fork}$: it defines a server which replicates
$M$, and returns the channel by which it may be used (of type $\gvdual{\gvserver{S}} =
\gvservice{\gvdual{S}}$).
...
% %
% The remaining rules are admissible (though not all depth-preserving admissible~\cite{TroelstraBook},
...
\[
  \bl
  Zeros : Source \uto \outterm \\
  Zeros = \cofix\:f\:c = \gvcase{c}{\clabel{next}\,c.\gvlet{c}{\gvsend{0}{c}}{f\:c};
                                        \clabel{stop}\,c.c}
  \el
\]
\normalsize
...
\small\[
\bl
\key{let}\:c = \gvfork{c}{Zeros\:c}\:\key{in} \\
...
\clabel{stop}:\:\outterm \}$ and $f$ the type $Source \uto \outterm$ in the body of $Zeros$, but
...
has type $X \uto \outterm$ instead of $\nu G \uto \outterm$. The body of $\cofix$ is thus required
to provide exactly one step of the corecursive behavior.
...
\small\[
\bl
  Nats : Source \uto \outterm \\
  Nats = \cofix\:nats\:c\:x = \mkwd{case}\:c\:\{
    \ba[t]{@{}l@{}l@{}}
...
\small\[
\bl
\key{let}\:c = \gvfork{c}{Nats\:c\:0\:1}\:\key{in} \\
\key{let}\:(x, c) = GetNext\:c\:\key{in}\:
\key{let}\:(y, c) = GetNext\:c\:\key{in}\:
\key{let}\:(z, c) = GetNext\:c\:\key{in} \\
\key{let}\:c = \gvselect{\clabel{stop}}{c}\:\key{in}\:(x,(y,z)) \\
...
\small\[
\bl
  Fibs : Source \uto \outterm \\
  Fibs = \cofix\:fibs\;c\:m\:n = \mkwd{case}\:c\:\{
    \ba[t]{@{}l@{}l@{}}
...
We now define a basic multi-function calculator. We aim to implement a process that accepts a stream
of addition and multiplication requests, and can at any time provide the accumulated result:
%
\small\[
...
% SL: space stealing
%
% (As with \mugv, we omit polymorphism as it is orthogonal to the rest of the development.)
...
X.\cpdual{(F(\cpdual{X}))}.$ Similarly to \mugv, $\cpdual{(F(A))} = \cpdual{F}(\cpdual{A})$.
%
We let $\Gamma, \Delta$ range over type environments. The judgement $\cptyp{P}{\Gamma}$ states that
process $P$ uses channels $\Gamma$.
%
Typing rules for \mucp terms are given in Fig.~\ref{fig:cp-typing}.  We write $fv(P)$ for the free
...
\shade{\inferrule{\cptyp{P}{\Gamma,y:A} \\
                 ...
                 {\cptyp{\corecf{F}{x}{y}{P}{Q}}{\Gamma,x:\nu F}}}
\end{mathpar}
\vspace{-1mm}
...
%% \inferrule{\vdash \cpdual{A}, F(A) \\ \vdash \Gamma, A}{\vdash \Gamma, \nu F}
%% \end{mathpar}\normalsize%
The bottom two rules are the typing rules for the \mucp terms $\rec{x}.P$ and
...
%% We can show that cut reduction preserves typing.
...
%% As cuts are interpreted as communication, Theorem~\ref{thm:cut-reduction} implies deadlock and
%% livelock freedom. Note that while we commute cuts under prefixes, we do not perform cut reduction
%% under prefixes. This corresponds to the usual notion of process communication. Were we to extend our
...
natural numbers (we could implement $Nat$ within \mucp, but choose not to due to lack of space). In
particular, assume there exists a proposition $Nat$, such that $\cpdual{Nat}$ is subject to
contraction and weakening, and terms:
%
\small\[
\begin{eqs}
Zero_x &\vdash& x:Nat \\
...
We can now use fixed points in \mucp to encode streams of naturals.
%
\small\begin{equations}
...
%%
%% SL: decided to comment the following out because I think it's too confusing here as is.
%%
%%  Only one of these rules (unrolling $\mu G$ to $G (\mu G)$)
%% corresponds directly to a term of \mucp; the remainder correspond to derivations in \mucp.
%% , an observation that motivates our argument (in \secref{cptogvpi}) that they
%% are also derivable in \mugv.
%
...
%; none of the remaining unfolding are used in the typing derivation of $\togv{P}$.
...
%% %%   The final three rules of Fig.~\ref{fig:gv-typing} are admissible.
%% %% \end{lemma}
%% %% This lemma follows from the composition of the $\tocp{-}$ and $\togv{-}$ translations, as the result
%% %% does not use any of the rules in question.
%% %
%% Note, however, that two of the rules do introduce additional term structure, as they rely on
%% functoriality.

%% the translations of the unrolling of $\nu G$ into $G (\nu G)$ and the rolling of
%% $G (\mu G)$ into $\mu G$ introduces additional term structure (with depth bounded by the size of
%% $G$).
...
\label{sect:bib}
\bibliographystyle{abbrvnat} \bibliography{main}
...
%%  LocalWords:  livelock Caires Pfenning Wadler Vasconcelos Baelde GayVasconcelos CairesPfenning
%%  LocalWords:  intuitionistic CP GV LindleyM Toninho Yoshida DemangeonH duals Bernardi Wadler's
%%  LocalWords:  gray exponentials polymorphism corecursion corecursive subformulas iff pos un fv
%%  LocalWords:  homomorphic equirecursive cptogvpi versa dually th behavior replicable Baelde's et
%%  LocalWords:  recognize corec coinvariant Nats HGV dualization tuple encodings ary pointwise al
%%  LocalWords:  dualized functoriality Takeuchi Girard Abramsky BellinScott Kobayashi Dardha inl
