...
\usepackage{amsmath,amssymb,infer,mathwidth,stmaryrd,xcolor}
...
{\small\begin{gather*}
\begin{aligned}
  \cpdual{(A \otimes B)} &= \cpdual{A} \parr \cpdual{B} & \cpdual{1} &= \bot & \cpdual{(A \oplus B)} &= \cpdual{A} \with \cpdual{B} & \cpdual{\top} &= 0 \\
  \cpdual{(A \parr B)} &= \cpdual{A} \otimes \cpdual{B} & \cpdual{\bot} &= 1 & \cpdual{(A \with B)} &= \cpdual{A} \oplus \cpdual{B} & \cpdual{0} &= \top
\end{aligned}\\
\begin{aligned}
...
\todo{Is it useful/interesting to show that $\cpdual{(F(A))} = \cpdual{F}(\cpdual{A})$?} \\
...
\begin{proof}
  Suppose that $F = X.B$; the proof is by induction on the structure of $B$. \qed
\end{proof}

The typing rules are mostly unchanged from previous presentations of CP:

{\small\begin{gather*}
\infbox{\irule;{\cptyp{\link{x}{y}}{x:A,y:\cpdual{A}}}}
\isp
\infbox{\irule{\cptyp{P}{\Gamma,x:A}}
              {\cptyp{Q}{x:\cpdual{A},\Delta}};
              {\cptyp{\cut{x}{A}{P}{Q}}{\Gamma,\Delta}}}
\isp
\infbox{\irule;{\cptyp{x[].0}{x:1}}}
\\
\infbox{\irule{\cptyp{P}{y:A,\Gamma}}
              {\cptyp{Q}{x:B,\Delta}};
              {\cptyp{x[y].(P \mid Q)}{x:A \otimes B,\Gamma,\Delta}}}
\isp
\infbox{\irule{\cptyp{P}{y:A,x:B,\Gamma}};
              {\cptyp{x(y).P}{x:A \parr B,\Gamma}}}
\isp
\infbox{\irule{\cptyp{P}{\Gamma}};
              {\cptyp{x().P}{x:\bot,\Gamma}}}
\\
\infbox{\irule{\cptyp{P}{x:A,\Gamma}};
...
\isp
\infbox{\irule;{\cptyp{\case{x}{}}{x:\top,\Gamma}}}
\\
\infbox{\irule{\cptyp{P}{x:F(\mu F), \Gamma}};
