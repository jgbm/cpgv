...
are also derivable in \mugv. The translation of $\cofix$ is a corecursive process with coinvariant
$x$ in which $p$ abstracts re-esatablishing the coinvariant at the end of each step.
...
and \tkwd{corec}. The translation $\rec{x}.P$ relies on exactly one use of the rule that unfolds
$\mu F$ to $F (\mu F)$ applied to the variable $x$; none of the remaining unfoldings are used in the
typing derivation of $\togv{P}$. The translation of \tkwd{corec} must simulate the expansion of the
term $Q$, using $p$ to re-establish the coinvariant after each expansion, and thus depends on the
...
deadlock, and livelock. We identify several areas of future work.
%
