...
introduced in a way in which both ends of a channel are treated as least fixpoints, a mismatch with
...
\citet{Toninho13,Toninho14} also treat recursion in a system inspired by a propositions-as-types
...
\citet{Bernardi13} observes that the standard treatment of recursive session types fails to preserve
duality in the case that the recursive variable appears within the type of a value that is input or
output. Their solution relies on an unusual form of substitution that applies only to the
problematic positions within a session type, while our solution avoids the problem while using only
standard forms of substitution.
...
* Define notation for: FV, fn, substitution

* Mention polymorphism

* Highlight Xs

* do we need commutativity of link in structural equivalence relation?

* Consistent way of saying: race-free, deadlock-free, livelock-free.

* Make last part of conclusion a little more interesting.

* Example of using Nats paralleling corresponding mugv example.

* Cite Simon et al's Concur submission
