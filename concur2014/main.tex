...
being sent from $P$ to $Q$; thus, its type is dual in $P$ and $Q$. As messages are not dual in
...
the addition of second-order polymorphism, as studied by \citet{Wadler12} for CP; the addition of
the MIX$_0$ and MIX$_2$ rules, providing additional notions of parallel composition and unifying the
types $\outterm$ and $\interm$; and identify natural extensions of CP that give rise to
non-determinism, and thus allow programs to exhibit more interesting concurrent behavior, while
preserving the underlying connection to linear logic.
