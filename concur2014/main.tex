...
\begin{abstract}
  ...
  preserve the freedom from races, deadlock, and livelock that are a hallmark of the
  propositions-as-types approach, and avoid some problems with recursion that
  appear in other systems.  We treat two systems, one based on classical linear logic, derived from
  work of Caires and Pfenning and of Wadler, and one based on a linear functional language, derived
  from work of Gay and Vasconcelos. Our treatment of recursion is inspired by Baelde.
...
introduced in a way where both ends of a channel are treated as least fixpoints, a mismatch with the
treatment of the other connectives.  Here, we introduce a notion of recursion for session types that
restores the traditional notion that least fixpoints are dual to greatest fixpoints.

Session types were introduced as a type system for process calculi by \citet{Honda93}, further
developed by \citet{Honda98}, and reformulated in the context of a functional language with linear
types by \citet{GayVasconcelos10}.  A propositions-as-types connection between session types and
intuitionistic linear logic was uncovered by \citet{CairesPfenning10} and adapted to classical
linear logic by \citet{Wadler12}.  Wadler defines two systems, a process calculus CP (inspired by
Caires and Pfenning) and a linear functional language GV (inspired by Gay and Vasconcelos), and
presents a type-preserving translation from GV to CP. Subsequently, \citet{LindleyM14} extended GV
to support translation in the other direction, giving type-preserving translations not only from GV
to CP but also from CP to GV.  A hallmark of the process-as-types approach is that it yields systems
guaranteed free of races, deadlock, and livelock, and both CP and GV benefit from these properties.

There is much previous work that supports recursive session types, but this work has not previously
been carried over to the propositions-as-types approach, and suffers from the mismatch mentioned
above.  Here, drawing on work of \citet{Baelde12}, we extend both CP and GV with recursive session
types, taking least and greatest fixpoints as duals.  We show that the type-preserving translations
extend to the new systems, and that the new systems are still guaranteed free of races, deadlock,
and livelock.

This paper makes the following contributions.
\begin{itemize}
\item Section~\ref{sec:mugv} extends GV to support recursion, yielding \mugv; and demonstrates its
  power with a series of examples including streams, a calculator, and a translation of a bit stream
  example due to \citet{Toninho13}. In \mugv, unlike in previous session type systems, least fixpoints
  are dual to greatest fixpoints.
\item Section~\ref{sec:mucp} extends CP to support recursion, yielding \mucp, and present a
  translation of some of the same examples. We show \mucp, like previous systems based on
  propositions as types, is guaranteed free of races, deadlock, and livelock.
...
\todo{The introduction should contain a sentence to compare with the recent work by Gay and
  colleagues.}
...
% There are two variations on the closed channel ($\interm$) and ($\outterm$); these arise from our
% interpretation of session types in classical linear logic, where there is no self-dual proposition
% corresponding to closed channels.
...
$\gvserver{S}$ is the server providing such a service (unfortunately, we cannot use the traditional
! and ? of linear logic to denote replication because these are already used for input and output).
Finally, we include session variables and
...
Structural rules
\begin{mathpar}
\un{(T \uto U)}  \qquad  \un{(\gvservice{S})}  \qquad  \un{(\interm)}
\end{mathpar}
\begin{mathpar}
...
$\gvservice{S}$ as $!$ and $?$ are already used for input and output.)  The rule for
$\gvserve{x}{M}$ parallels that for $\tkwd{fork}$: it defines a server which replicates $M$, and
...
\[\small\begin{array}{@{}l@{~}r@{~}c@{~}l@{}}
  \text{Types} & A,B & ::= & A \otimes B \mid A \parr B \mid \bot \mid 1 \mid A \oplus B \mid A \with B \mid 0 \mid \top \\
   & & \mid & \cpbang{A} \mid \cpquery{A} \mid X \mid \cpdual{X} \mid \shade{\mu F \mid \nu F} \\ \shaderow
...
coinvariant at the end of each iteration.  In both, we rely on syntactic sugar for output: $x[y].P
\equiv x[y'].(\link{y}{y'} \mid P)$.
