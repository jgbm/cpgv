...
Finally, we include session variables and their duals ($X,\gvdual{X}$), and types corresponding to
bounded ($\mu X.S$) and unbounded ($\nu X.S$) recursion. If $G$ is an operator $X.S$, we write
$G(S')$ for the substitution instance $S[S'/X]$.
...
initiation; like Wadler's GV, but unlike LAST (and much other work on process calculi), \mugv is
deterministic. The rule for $\gvserve{x}{M}$ parallels that for $\tkwd{fork}$: it defines a server
which replicates $M$, and returns the channel by which it may be used (of type
$\gvdual{\gvserver{S}} = \gvservice{\gvdual{S}}$).
...
defined typing rules up to unfolding of recursive types. We present explicit rules in order to
simplify the translation into \mucp, in which these rules will correspond to terms implementing the
equivalences.
%
...
For the purposes of our examples, we assume an extension of \mugv with a type of naturals
($Nat$), suitable constants ($0,1,\dots$), and addition ($+$).
...
There are three rules for types $\cpquery{A}$; note that, uses of weakening and contraction implicit
in \mucp terms.  Finally, since the process $P$ proving $A$ may be replicated arbitrarily, each
channel that $P$ uses must be replicatable as well.  We write $\cpquery{\Gamma}$ to require that all
types in $\Gamma$ be of the form $\cpquery{B}$.
...
\mugv sessions, no corresponding duality appeared in the typing rule for $\lrkwd$.
...
Let $\topi{\Phi}$ be the pointwise extension of $\topi{T}$.  It is straightforward to verify that
our translation preserves typing.
\begin{theorem}
...
this difficulty.  They propose a different solution than ours, however, and we hope to investigate
...
$\pi$-calculus processes.  As a result, they are able to show that well-typed processes are race
free and deadlock free, by analogy with corresponding cut-elimination results for ILL.
\citet{Toninho13} demonstrate an embedding of their calculus within a functional language; the
