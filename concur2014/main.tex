...
\title{Duality, Recursion, and Session Types\vspace{-3mm}}
\author{J. Garrett Morris \and Sam Lindley}
\institute{The University of Edinburgh \\
           \email{\{Garrett.Morris,Sam.Lindley\}@ed.ac.uk} \vspace{-5mm}}
...
The rule for $\gvserve{x}{M}$ parallels that for $\tkwd{fork}$: it defines a server that replicates
$M$, and returns a channel by which it may be used (of type $\gvdual{\gvserver{S}} =
\gvservice{\gvdual{S}}$).

The novelty of \mugv is in its recursive and corecursive channels. The rule for $\mu$ allows a
recursive sesion type to be unfolded.
% SL: possibly restore this?
%
%% The first two typing rules allow recursive session types to be used as their unfoldings.
...
translations are as in our previous work. The elimination of a least fixed point in GV is translated
to a cut against the introduction of the least fix point. The translation of $\cofix$ is a
...
* try to be consistent with order of presenting types everywhere
...
* consider switching the order of the arguments to map to make it's definition nicer
