...
\begin{abstract}
  A Curry-Howard correspondence between session types and linear logic was first demonstarted by
  Caires and Pfenning for intuitionistic linear logic, and later adapted to the classical setting by
  Wadler.  These correspondences give both a logical interpretation of session types and computation
  interpretation of linear logic proofs, and can be used to establish properties such as
  deadlock-freedom and termination of well-typed processes.  However, neither system accomodates
  recursive sessions, as might be used to represent stateful servers or inductive data types.

  We extend Wadler's correspondence to include recursive processes and sessions.  We present a
  system with equirecursive session types and a (limited) form of recursive definition, and show the
  resulting types and processes are in correspondence with the propositions and proofs of Baelde's
  extension of linear logic with greatest and least fixed points.  Unlike existing approaches to
  recursive processes, our system introduces both recursive and corecursive processes, and exploits
  their duality to maintain termination and deadlock freedom even in the presence of unbounded
  processes.
\end{abstract}
