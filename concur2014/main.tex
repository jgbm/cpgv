...
The remaining rules are admissible, albeit at the cost of introducing additional computation.
...
many examples, including all of those of \citet{Toninho13}. All of the code is available online:
...
\]\normalsize%
It is given in Fig.~\ref{fig:fromcp}. The interesting cases are for \tkwd{rec} and \tkwd{corec}. The
translation $\rec{x}.P$ relies on exactly one use of the rule that unrolls $\mu F$ to $F (\mu F)$
applied to the variable $x$.
