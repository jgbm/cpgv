...
%
The fixed point construct $\gvfix{f}{c\,\vec{x}}{M}$ is used to define corecursive sessions.
...
\todo{This example is useless because we cannot actually use a stream of ints without introducing a
  way of terminating the stream! ($\nu X.\with \{\clabel{more}: !Int.X, \clabel{end}: \outterm \}$)}

\todo{Change the example to match the equivalent CP one}
...
\[
  ...
  makeZeros = \cofix\:f\:c = \gvlet{d}{\gvsend{0}{c}}{f\:d} \\
  \el
\]
...
\]
%
We restrict the typing rule for $\cofix$ in order to ensure productivity. We use a fresh type
...
\[
  \gvfix{nats}{c\:x}{\gvlet{c}{\gvsend{x}{c}}{nats\:c\:(x + 1)}}
\]
...
\[
  \gvfix{fibs}{c\:m\:n}{\gvlet{c}{\gvsend{m}{c}}{fibs\:c\:n\:(m + n)}}
\]
...
\[
\bl
\key{let}\:fibs = (\gvfix{fibs}{c\:m\:n}{\gvlet{c}{\gvsend{m}{c}}{fibs\:c\:n\:(m + n)}})\:\key{in} \\
\gvfork{c}{fibs\:c\:0\:1} \\
\el
\]
...
processes, as we will demonstrate for \mucp~\secref{peano}---but does significantly streamline the
presentation.
...
of $x$.  The units of $\otimes$ and $\parr$, $1$ and $\bottom$ are interpreted as nullary output and
