...
expandP ds = ex
    where ex (ProcVar v)           = do (vs, p) <- getBinding v (procs ds)
          ...
          ex (Inl x p)             = liftM (Inl (exn x)) (ex p)
          ex (Inr x p)             = liftM (Inr (exn x)) (ex p)
          ex (Case x p q)          = liftM2 (Case (exn x)) (ex p) (ex q)
...
expandT ds (Plus t u)     = liftM2 Plus (expandT ds t) (expandT ds u)
expandT ds (With t u)     = liftM2 With (expandT ds t) (expandT ds u)
